...
static void test_find_checks_size(void) {
    SECTION("find checks size");
    freelist_init();
...
static void test_larger_block_satisfies(void) {
    SECTION("larger block satisfies");
    freelist_init();
...
static void test_check_consistency(void) {
    SECTION("check consistency");
    freelist_init();
    CHECK(freelist_check() == 0);

    block_t *a = make_free_block(8, 64);
    block_t *b = make_free_block(1024 + 8, 128);
    freelist_insert(a);
    freelist_insert(b);
    CHECK(freelist_check() == 0);
...
int main(void) {
    printf("running freelist interface tests\n");
    test_insert_find_basic();
    test_find_does_not_remove();
    test_find_checks_size();
    test_larger_block_satisfies();
    test_remove_middle();
